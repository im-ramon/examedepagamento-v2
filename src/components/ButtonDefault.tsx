...
export function ButtonDefault({ type, children, isLoading, variant, color, click, disabled }: ButtonDefaultProps) {
    return (
        ...
        >
            ...
            {color == 'yellow' && <><span className={`absolute inset-0 w-full h-full bg-gradient-to-br from-yellow-300 via-yellow-400 to-yellow-300`}></span><span className={`absolute bottom-0 right-0 block w-64 h-64 mb-32 mr-4 transition duration-500 origin-bottom-left transform rotate-45 translate-x-24 bg-yellow-600 rounded-full opacity-30 group-hover:rotate-90 ease`}></span></>}
